...
# python3 -m PD_PTB_Benchmark_without_regularization
...
from PTB_models import new_pd_NN_individual_without_regularization
...
def individual_to_params(
	...
	def Parameter_class_observer(population, history):
		...
		batch_size = 64
		tIndices = np.random.choice(84558 - 1, size = (batch_size*10, ), replace=False)
...
def create_Parameters_NN_object(pop_size, randomization, CV_selection, rr):
	...
	object = individual_to_params(pop_size, lambda: new_pd_NN_individual_without_regularization(), NN_randomizer_manual_loss, NN_optimizer_manual_loss, observer, randomization=randomization, CV_selection=CV_selection, rr=rr, history=history)
	object.population, model_num = object.population(pop_size) # initializing population
...
if __name__ == "__main__":
	...
	for i in range(len(SEED)):
		...
		with open('../pd_PTB_benchmark_without_regularization.csv', 'a', newline = '') as file:
