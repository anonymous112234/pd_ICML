...
# cd Benchmark_Tests
# python3 -m PD_Stanford_Dogs_Benchmark_without_regularization
...
from NN_models_Stanford_Dogs import new_pd_NN_individual_Stanford_Dogs_without_regularization
...
def NN_optimizer_manual_loss(NN_object, batches, batch_size, epochs):
	...
	indices = np.random.choice(len(train_images) - 1, size = (batch_size*batches, ), replace=False)
	vIndices = np.random.choice(len(validation_images) - 1, size = (batch_size*10, ), replace=False)
...
def evaluator(NN_object):
	...
	np.random.seed(0)
	eIndices = np.random.choice(len(validation_images) - 1, size = (batch_size*25, ), replace=False)
...
def individual_to_params(
	...
	def Parameter_class_observer(population, history):
		...
		batch_size = 64
		tIndices = np.random.choice(len(validation_images) - 1, size = (batch_size*10, ), replace=False)
...
def create_Parameters_NN_object(pop_size, randomization, CV_selection, rr):
	...
	object = individual_to_params(pop_size, lambda: new_pd_NN_individual_Stanford_Dogs_without_regularization(), NN_randomizer_manual_loss, NN_optimizer_manual_loss, observer, randomization=randomization, CV_selection=CV_selection, rr=rr, history=history)
...
# # Define functions to split images and labels
# def split_images_labels(image, label):
#     return image, label
...
# # Split the training set into images and labels
# ds_train_images_labels = ds_train.map(split_images_labels)

# # Extract images and labels separately
# ds_train_images = ds_train_images_labels.map(lambda image, label: image)
# ds_train_labels = ds_train_images_labels.map(lambda image, label: label)

# print(len(ds_train_images))



# Split the training set into images and labels
train_images = ds_train.map(lambda image, label: image)
train_labels = ds_train.map(lambda image, label: label)

# Split the test set into images and labels
test_images = ds_test.map(lambda image, label: image)
print(test_images[0])
test_labels = ds_test.map(lambda image, label: label)
...
# train_data = prepare(ds_train)
# test_data  = prepare(ds_test)


validation_images = test_images[:int(len(test_images) * 0.5)]
validation_labels= test_labels[:int(len(test_labels) * 0.5)]

test_images = test_images[int(len(test_images) * 0.5):]
...
if __name__ == "__main__":
	...
	for i in range(len(SEED)):
		...
		with open('../pd_CIFAR100_benchmark_without_regularization.csv', 'a', newline = '') as file:
